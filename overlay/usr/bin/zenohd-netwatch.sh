...
# 快照 zenohd 每次启动时 end0 的 carrier 状态与 IPv4/IPv6 地址集,之后清单一旦变化
...
snapshot() {
    STATE=$(cat /sys/class/net/end0/operstate 2>/dev/null || echo missing)
    V4=$(ip -4 addr show end0 2>/dev/null | grep -o 'inet [0-9.]*' | sort | tr '\n' ',')
    V6=$(ip -6 addr show end0 2>/dev/null | grep -o 'inet6 [0-9a-fA-F:]*' | sort | tr '\n' ',')
    echo "state:$STATE|v4:$V4|v6:$V6"
}
...
while true; do
    ...
    if [ "$CUR" != "$BASE" ]; then
        ...
        CUR2=$(snapshot)
        if [ "$CUR2" = "$CUR" ]; then
            echo "end0 网络清单变化: [$BASE] -> [$CUR],重启 zenohd 以刷新组播/locator"
